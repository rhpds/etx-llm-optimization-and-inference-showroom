...
First, we'll clone the GuideLLM pipeline repo. Then we'll clone the benchmark pipeline repo, apply the PVC, task and pipeline. We'll also create an s3 bucket in Minio where the pipeline will upload the benchmark results.
...
TIP: If the pipeline run fails with `HTTPStatusError: Server error '503 Service Unavailable'` for the provided INFERENCE_ENDPOINT, then check and fix the relevant network policy, example maybe vllm namespace have 3scale network policy which needs to be removed!

[NOTE]
====
You can track the PipelineRun progress:

[source,console,role=execute,subs=attributes+]
----
tkn pipelinerun logs guidellm-benchmark-pipeline-run-<#> -f -n vllm
----
Replace # with the actual piplelinerun name
