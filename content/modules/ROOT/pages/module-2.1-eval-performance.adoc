...
* Deploy helm if its not available on your RHEL instance (bastion host)
----
...
Alternatively, you can also curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-4 | bash
----

* Deploy tkn cli if its not available on your RHEL instance (bastion host)
----
If you are using the latest releases of Fedora or RHEL or CentOS, you may use the TektonCD CLI unofficial copr package repository instead or can use this url to download and deply tkn cli (https://docs.redhat.com/en/documentation/red_hat_openshift_pipelines/1.17/html/pipelines_cli_tkn_reference/installing-tkn):
