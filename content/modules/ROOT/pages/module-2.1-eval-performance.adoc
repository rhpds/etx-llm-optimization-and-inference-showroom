...
* Tekton pipeline: Refer to this https://github.com/ishuverma/guidellm-pipeline.git[repository] which was forked from above repository.

[NOTE]
====
Install the Tekton CLI using the appropriate command for your platform:

*For Lab Showroom terminal (Linux):*
...
*For Mac users (Darwin):*

[source,bash]
----
curl -sL $(curl -s https://api.github.com/repos/tektoncd/cli/releases/latest | grep "browser_download_url.*_Darwin_all.tar.gz" | cut -d '"' -f 4) | sudo tar -xz -C /usr/local/bin tkn
----
...
== Understanding GuideLLM Configuration

Before setting up the pipeline, let's review the GuideLLM configuration options to understand what parameters we'll be using.

=== GuideLLM Arguments

* Peruse the available GuideLLM https://github.com/vllm-project/guidellm[configuration options].
...
The `--rate-type` parameter defines the type of benchmark to run. By default GuideLLM will do a sweep of available benchmarks, but you may choose to isolate specific benchmark tests.

*synchronous*::
Runs a single stream of requests one at a time. `--rate` must not be set for this mode.

*throughput*::
Runs all requests in parallel to measure the maximum throughput for the server (bounded by `GUIDELLM__MAX_CONCURRENCY` config argument). `--rate` must not be set for this mode.

*concurrent*::
Runs a fixed number of streams of requests in parallel. `--rate` must be set to the desired concurrency level/number of streams.
...
*sweep*::
Automatically determines the minimum and maximum rates the server can support by running synchronous and throughput benchmarks, and then runs a series of benchmarks equally spaced between the two rates. The number of benchmarks is set by `--rate` (default is 10).
...
Now that we understand the GuideLLM configuration options, let's set up the pipeline. We'll clone the GuideLLM pipeline repo, apply the PVC, task and pipeline resources, and create an S3 bucket in MinIO where the pipeline will upload the benchmark results.
...
git clone https://github.com/ishuverma/guidellm-pipeline.git
----
...
- Format: Provide data in JSONL or CSV format or any of the https://github.com/vllm-project/guidellm/blob/main/docs/guides/datasets.md[supported formats].
...
. Ensure that the redhat-ai-services helm repo is available:
+
[source,console,role=execute,subs=attributes+]
----
...
[NOTE]
====
Make sure the granite-8b-instruct model is deployed on OpenShift AI. If you need to deploy it, you can use:

`helm upgrade -i granite-8b-instruct redhat-ai-services/vllm-kserve --version 0.5.11 -f workshop_code/deploy_vllm/vllm_rhoai_custom_1/values.yaml -n vllm`
====

. Remove any restrictive network policies that may block the benchmark pipeline from accessing the inference endpoint:
+
[source,console,role=execute,subs=attributes+]
----
oc delete networkpolicy 3scale -n vllm --ignore-not-found=true
----
...
GuideLLM writes several output files that are uploaded to Minio. Each one focuses on a different layer of analysis, ranging from a quick on-screen summary to fully structured data for dashboards and regression pipelines.
...
image::benchmark_results_file_in_minio-1.png[benchmark results in Minio]

## Inspect Results

### benchmarks.txt

The txt file provides lightweight summary with high-level statistics for each benchmark in the run. It's useful for quick checks to confirm that the server responded correctly, the load sweep completed, and the system behaved as expected. Additionally, the output tables can be copied and pasted into spreadsheet software using | as the delimiter. The sections will look similar to the following:

image::benchmark_results_txt.png[benchmark results]

Download the benchmark results from the *guidellm-benchmark* bucket in Minio.

 and open the *benchmark-<TIMESTAMP>.txt* in a text editor.
...
The HTML report provides a visual summary of results, including charts of latency distributions, throughput behavior, and generation patterns. It's ideal for quick exploration or sharing with teammates without requiring them to parse JSON. You can view the html file in the browser and set SLO values.

image::benchmark_results_html.png[benchmark results in html]
